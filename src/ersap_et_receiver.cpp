...
bool receiveEvents(e2sar::Reassembler& reassembler, ETSystem& et) {
    ...
    while (keep_receiving) {
        ...
        // Build augmented buffer: data_id stored as double prepended before original payload.
        // Layout: [data_id (8 bytes)] [original payload (event_size bytes)]
        size_t augmented_size = sizeof(double) + event_size;
        std::vector<uint8_t> augmented_buffer(augmented_size);
        double data_id_as_double = static_cast<double>(data_id);
        std::memcpy(augmented_buffer.data(), &data_id_as_double, sizeof(double));
        std::memcpy(augmented_buffer.data() + sizeof(double), event_buffer, event_size);
